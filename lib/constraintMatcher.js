...
var lang = {
    ...
    getArguments: function (rule) {
        var ret = [];
        var rule2 = rule[2];

        if (rule2 === "identifier") {
            //its an identifier so stop
            return [ (rule[0] + identSuffix) ];
        } else if (rule2 === "function") {
            ret = ret.concat(this.getArguments(rule[0])).concat(this.getArguments(rule[1]));
        } else if (
			rule2 !== "string" &&
            rule2 !== "number" &&
            rule2 !== "boolean" &&
            ...
            //its an expression so keep going
            if (rule2 === "prop") {
                ret = ret.concat(this.getArguments(rule[0]));
                if (rule[1]) {
                    var propChain = rule[1];
                    //go through the member variables and collect any identifiers that may be in functions
                    while (isArray(propChain)) {
                        if (propChain[2] === "function") {
                            ret = ret.concat(this.getArguments(propChain[1]));
                            break;
                        } else {
                            propChain = propChain[1];           
                        }
                ...
                if (rule[1]) {
                    ret = ret.concat(this.getArguments(rule[1]));
                }
            }
        }
		else {
    ...
    toConstraints: function (rule, options) {
        ...
        if (rule2 === "and") {
			...
			if( rule2 === 'function' && queries && queries[rule[0][0]] ) {	// fnName = rule[0][0], it's a query..
				ret.push(new atoms.QueryConstraint(rule, options));
			}
			else {
				switch (rule2) {
					case "eq":
...
var toJs = exports.toJs = function (rule, scope, alias, equality, wrap) {
    /*jshint evil:true*/
	var  closureVars = ["var indexOf = definedFuncs.indexOf; var hasOwnProperty = Object.prototype.hasOwnProperty;"], 
		js = lang.parse(rule),
		funcVars = [],
		closureBody, f, fnSig, vars, closureVars;   
	//
    scope = scope || {}; 
	...
	//
	closureBody = closureVars.join("") + "return function matcher" + (matcherCount++) + (!equality ? "(fact, hash){" : "(fact){") + funcVars.join("") + " return " + (wrap ? wrap(js) : js) + ";}";
	f			= new Function("definedFuncs, scope", closureBody)(definedFuncs, scope);
    return f;
};

var identSuffix = ':~ident~:';

var toJsFunc = function (rule, scope, alias, equality, wrap) {
    /*jshint evil:true*/
	var  closureVars = ["var indexOf = definedFuncs.indexOf; var hasOwnProperty = Object.prototype.hasOwnProperty;"], 
		js = lang.parse(rule),
		funcVars = [],
		queries, queryVar, closureBody, f, querySig, queryParams, fnSig, args, closureVars;   
	//
    scope = scope || {}; 
	queries = scope.queries;
	args = lang.getArguments(rule);
    //
	extd(args).filter(function (v) {
		var	idx = 'string' === typeof v ? ( v.indexOf(identSuffix) ) : undefined,
			ret;
		//
		if( idx > 0 ) {						// it's an identifier...
			v = v.substr(0,idx);
			ret = ["var ", v, " = "];
			if (definedFuncs.hasOwnProperty(v)) {
				ret.push("definedFuncs['", v, "']");
			...
			return false;				// skip it's not a closure var, scope or define
		}
        ret.push(";");
        closureVars.push(ret.join(""));
        return false;
        ...
        var  idx = v.indexOf(identSuffix)
			,ret;
		v = v.substr(0,idx);
		ret = ["var ", v, " = "];
		if( queries && queries[v] ) {
			queryVar = v;
			ret.push("this.getQuery('" + v + "')");
		}
        else if (equality || v !== alias) {
            ret.push("fact." + v);
        } else if (v === alias) {
            ret.push("hash.", v, "");
        }
	...
	//
	if( queryVar ) {			// queryVar is always @ 0, with remaining params...->
		queryParams = [];
		for(var i=1; i < args.length; i++) {
			var arg = args[i],
				idx = ('string' === typeof arg) ? arg.indexOf(identSuffix) : undefined;
			//
			arg = idx ? (arg.substr(0,idx)) : arg;
			queryParams.push(arg);
		}
		querySig	=  [queryVar,'(', queryParams.join(','), ');'];
		closureBody = ['return function(fact,hash){ ', funcVars.join('\n'), ('return ' + querySig.join('')), ' }' ];
		fnSig		= closureBody.join('\n');
		f			= new Function('definedFuncs', 'scope', fnSig);
		f			= f(definedFuncs, scope);
		return f;	
	}
	else {
		closureBody = closureVars.join("") + "return function matcher" + (matcherCount++) + (!equality ? "(fact, hash){" : "(fact){") + funcVars.join("") + " return " + (wrap ? wrap(js) : js) + ";}";
...
exports.getSourceMatcher = function (rule, options, equality) {
    options = options || {};
	if( rule[2] === 'function' ) {
		return toJsFunc(rule, options.scope, options.alias, equality, function (src) {
			return "!!(" + src + ")";
		});
	}
	else {
