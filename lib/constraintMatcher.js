...
var lang = {
    ...
    toConstraints: function (rule, options) {
        ...
        if (rule2 === "and") {
			...
			if(rule2 === 'function' && queries[rule[0][0]] ) {	// fnName = rule[0][0], it's a query..
...
var toJs = exports.toJs = function (rule, scope, alias, equality, wrap) {
    /*jshint evil:true*/
	var queries, queryVar, closureBody, f, querySig, queryParams, fnSig;   
    var js = lang.parse(rule);
    scope = scope || {}; queries= scope.queries;
...
function helper(fact,hash) {
	debugger;
	var MsgFilter = this.getQuery('MsgFilter');
	var $maxLen = fact.$maxLen;
	var $filter = fact.$filter;
	var ret = MsgFilter($maxLen,$filter);
	return ret;
}
