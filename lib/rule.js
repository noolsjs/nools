...
////
//function isQuery(constraint,scope) {
//	var rule = parseConstraint(constraint)
//		,rule2 = rule[2]
//		,fnName;
//	if(!scope) {
//			debugger;
//	}
//	if( 'function' === rule2 && scope.queries) {
//		fnName = rule[0] ? rule[0][0] : undefined;
//		return fnName ? scope.queries[fnName] : undefined;
//	}
//}
...
var parsePattern = extd
	...
	.containsAt("query", 0, function(condition) {
		...
		*/
		function buildRHS(name, paramPatterns, scope) {
		...
		*/
		function buildQuery(name, paramPatterns, scope) {
			var wrapper		= ['return ', undefined]						// function QueryWrapper(paramPatterns) {  }
				,fnSig		= ['function ', name, '(', undefined, ')']
				,fnQuery	= [undefined, '{', 'var session = this, factIds = [];', 
				undefined, 
				' return session.queryResults["' + name + '"][factIds.join(":")];',
				' }'
				]
		...
		queryFn = buildQuery(params.name, patterns, scope);
