    ...
    .def(function (condition) {
        ...
        if (condition[4] && condition[4].from) {
			...
			function mergePatterns(patterns, merged, setAliases) {
				var flattened = _.flatten(patterns);
...
/**
    MB: chance to create extended Rule...
*/
var ruleFactory;
exports.setRuleFactory = function(factory) {
  ruleFactory = factory;
}
//
...
/**
    {
        ...
        ,constraints: [{
            ...
            ,reference: {....}
            ,from: expr
            ,conditional: [exists, not, or, query]
        },...]
        //
...
function ruleFromLiteral(obj) {
    ...
    var name         = obj.name
        ...
        ,constraint, rules;
    //
    if (extd.isArray(constraints) ) {
        //
        for (var i = 0; i < l; i++) {
            constraint = constraints[i];
            constraint.scope = scope;
            // create patterns from blobs, patterns may explode into an array of patterns; example: OR
            extd.forEach( patternFactory(constraint) , _mergePatterns(patterns));
        }
        //
        rules = extd.map(patterns, function (pattern) {
            var compPat = null;
            // builds pattern hierarchy
            for (var i = 0; i < pattern.length; i++) {
                if (compPat === null) {
                    compPat = new CompositePattern(pattern[i++], pattern[i]);
                } else {
                    compPat = new CompositePattern(compPat, pattern[i]);
                }
            }
            //
            if( !ruleFactory ) {
                return new Rule(name, options, compPat, action);
            }
            else  {
                return ruleFactory(name, options, compPat, action);
           }
        });
    } else {
        rules = extd.map(patternFactory(constraints), function (pattern) {
            if( !ruleFactory ) {
                 return new Rule(name, options, pattern, action);
            }
            else {
                 return ruleFactory(name, options, pattern, action);
               }
        });
    }
    return rules;
...
exports.ruleFromLiteral = ruleFromLiteral;

function patternFactory(constraint) {
    var tmp;
    //
    switch(constraint.conditional) {
        case 'or':
        ...
        default:
            return patternFromConstraint(constraint);
    };
}
...
*/
function patternFromConstraint(constraint) {
    var Type                = getParamType(constraint.type, constraint.scope)
        ,alias              = constraint.alias   || "m"              // i guess 'm' is as good as any...
        ,pattern            = constraint.pattern || "true"           // pattern is optional so true becomes expression
        ,parsedPattern      = (typeof pattern !== 'function') ? parser.parseConstraint(pattern) : pattern
        ,parsedFromPattern  = constraint.from ? parser.parseConstraint(constraint.from) : undefined
        ,reference          = constraint.reference                  // optional obviously; e.g. { property: alias, ...}
        ,options            = {scope: constraint.scope, pattern: pattern};

    //
    // this is weird... revisit, WTF handled somewhere else clearly a function is not a pattern so consider what happens here ?
    if (typeof constraint.pattern === 'function') {    
        return [constraint.pattern];
    }
    return (constraint.from) ? 
            [new FromPattern(Type, alias, parsedPattern, reference, parsedFromPattern, options)] 
           : [new ObjectPattern(Type, alias, parsedPattern, reference, options)];
}