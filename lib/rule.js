...
var getParamType = function (param) {
    ...
    if (comb.isDefined(param)) {
        if (param === String || param === "string" || param === "?string") {
            ret.type = String;
        } else if (param === Array || (comb.isArray(param) && param.length == 0) || param === "array" || param
                === "?array") {
            ret.type = Array;
            ...
            ret.literal = true;
            ret.type = param;
            if (!comb.isString(param) && !comb.isDate(param) && !comb.isBoolean(param) && !comb.isRexExp(param)
                    && !comb.isNumber(param) && !comb.isArray(param)) {
                ret.literal = false;
            }
        }
    }
