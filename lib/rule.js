...
var parsePattern = extd
	...
	.containsAt("query", 0, function (condition) {
		var params = condition[1]									// params.name, params.arguments, params.conditions
			,paramPattern, args = [], compPat, collectPattern, setOrientedAliases = [], patterns = [], queryFn, tmp;	
		/*
			The query function is call-able as a condition in an rule's LHS. 
			Query condition is converted into from queryFn(params);

		ex: list : Array from MsgFilter(/goodbye/, 5);			// the from modifier can be omitted for brevity
		//
		function wrapper(parameter-patterns) { 
			
			return function <query-name>(p1, p2...) { 
				var session = this;
				parameter-patterns['p1'].setParam(p1); 
				parameter-patterns['p2'].setParam(p2); 
				...
			}
		}
		*/
		...
		//
		// build ParamPattern instances from query args, store in LIFO order; network built bottom up...(collect pattern must be last node of rule! )
		...
		if( params.arguments ) {
			params.arguments.forEach(function(arg) {
				...
				args.unshift( new ParamPattern(getParamType(nCond[0], condition.scope),nCond[1], condition.scope) );
			});
		}
		else {		// there is no parameter produce a fake, the InitialParam ( like the initial fact ) since params are query; trigger/blocker
			args.push( new ParamPattern(Object, 'fake', condition.scope) );
		}
		//
		// now that we have the parameters we can build the Query Function
		// the query fn is kept in the flow kind of a special scope so it can be referenced in rule(s)
		tmp = [];
		args.forEach(function(pattern) { tmp.unshift(pattern); });		// we need the params in FIFO order
		queryFn = buildQuery(params.name, tmp);
		//
		// parse the (non parameter) query conditions ..., LIFO ordering
		...
		if( 1 === patterns.length ) {
			 compPat = new CompositePattern(patterns[0], collectPattern);
		}
		else { 
			patterns.unshift(collectPattern);				// patterns in LIFO order
			for (var i = 0; i < patterns.length; i++) {
				if (compPat === null) {				// left, right
		...
		//
		// take care of the parameters, they are in LIFO order
		for (var i = 0; i < args.length; i++) {
			compPat = new CompositePattern(args[i], compPat);
		}
		//
		compPat.queryFn = queryFn;
		compPat.queryName = params.name;
		return [compPat];
	})
...
function createQuery(name, options, conditions, cb) {
	...
	return createRule(name, options, conditions, cb)[0];
}
