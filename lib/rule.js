...
var parsePattern = extd
	...
	.containsAt("query", 0, function (condition) {
		...
		/**
			rhs for queries uses parameter aliases to build unique hashCode to store results
			queries with no parameters have a single 'auto' parameter,there is exactly one
			collection as the last fact for a given query, the collection fact
			the collection is stored in the sessoin and can be used in LHS of other rules
			or accessed directly via a session method
			...
			c.) the session must maintain a query hash for results by query name,that is furtherpatitioned by parameter hash-code(s)
		*/
		function buildRHS(name, paramPatterns) {
			var wrapper		= ['return ', undefined]						// function QueryWrapper(paramPatterns) { return fn(match) {...};  }
				,fnSig		= ['function ', name, '-RHS(match)'].join('')
				,fnQuery	= [fnSig, '{', 
											'var session = this; nFacts = match.factIds.length-1, factIds=[], existing, idx, hashCode, collection;', 
											 ...
											 'collection = session.queryResults[name][hashCode];',
											 'if(!collection) {',
											 'session.queryResults[name][hashCode] = match.facts[nFacts-1];',		// it's ok same collection each rhs invoke
											 '}',
									'}']
			...
			paramPatterns.forEach(function(param) {
				var paramExpr = ['idx = match.aliases.indexOf(',param.alias,'); factIds.push(match.factIds[idx]);'].join('');
		...
		*/
		...
		return [bottom];			// build from the bottom up
	})
