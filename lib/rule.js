...
var parsePattern = extd
	...
	.containsAt("query", 0, function (condition) {
		var params = condition[1]									// params.name, params.arguments, params.conditions
			,paramPattern, compPat, collectPattern, setOrientedAliases = [], patterns = [], queryFn, tmp, bottom;	
		/*
		...
		*/
		...
		if( params.arguments ) {
			...
			patterns.push( new ParamPattern(Object, 'fake', condition.scope) );
		}
		//
		...
		queryFn = buildQuery(params.name, patterns); //tmp);
		//
		...
		//
		tmp = [];
		patterns.push(collectPattern);				// patterns in LIFO order
