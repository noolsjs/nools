...
var utils = require("./util.js"),
    ...
    predicates = ["not", "or", "exists", "collect"],
...
var collectRuleRegExp = /^(\$?\w+)\s*:\s*collect *\((.*)\)$/;
var constraintRegExp = /(\{ *(?:["']?\$?\w+["']?\s*:\s*["']?\$?\w+["']? *(?:, *["']?\$?\w+["']?\s*:\s*["']?\$?\w+["']?)*)+ *\})/;
var fromRegExp = /(\bfrom\s+.*)/;
var parseRules = function (str) {
    var rules = [];
    var ruleLines = str.split(";"), l = ruleLines.length, ruleLine, ruleLineB, alias, innerParts, innerAlias;
    for (var i = 0; i < l && (ruleLine = ruleLines[i].replace(/^\s*|\s*$/g, "").replace(/\n/g, "")); i++) {
        if (!isWhiteSpace(ruleLine)) {
            ...
            if (predicateRegExp.test(ruleLine)) {
                ...
                alias       = parts[1];
                ruleLine    = parts[2];
                innerParts  = ruleLine.trim().match(ruleRegExp);
                innerAlias  = innerParts[1];
                rule.push('collect');
                ruleLineB = [alias, ' : ', 'Array ',' from [', innerAlias, ']'].join('');
                rule = rule.concat( parseRules(ruleLine)  );
                rule = rule.concat( parseRules(ruleLineB) );
                rules.push(rule);
                continue;
            }
...
var ruleTokens = {
    scope: (function () {
        ...
        return function (orig, context) {
            var src = orig.replace(/^scope\s*/, "").replace(/^\s*|\s*$/g, "")
                ,_tmp, obj, scope;
...
"query": function (orig, context, parse) {
    var src = orig.replace(/^query\s*/, "");
    var name = src.match(/^([a-zA-Z_$][0-9a-zA-Z_$]*|"[^"]*"|'[^']*')/);
    if (name) {
        src = src.replace(name[0], "").replace(/^\s*|\s*$/g, "");
        if (utils.findNextToken(src) === "{") {
            ...
            var rule = { name: name, options: {}, constraints: null, action: null };
            var body = utils.getTokensBetween(src, "{", "}", true).join("");
            rule.src = 'query ' + name + body;
            src = src.replace(body, "");
            var queryTokens = { scope: ruleTokens.scope, when: ruleTokens.when};
            parse(body.replace(/^\{\s*|\}\s*$/g, ""), queryTokens, rule);
            //
            rule.action = ['this.setQueryResults("', rule.name, '", facts);'].join('');
            rule.options.agendaGroup = ('Query_' + rule.name);
            context.rules.push(rule);
            //
            return src;
        } else {
