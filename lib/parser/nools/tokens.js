...
var utils = require("./util.js"),
	...
	fnSigParser = require('../function/parser.js');
...
var queryTopRegExp = /^([$A-Z_][0-9A-Z_$]*)\s*(\(.*\))\s*$/i; 
...
var topLevelTokens = {
	...
	"query": function (orig, context, parse) {
		var src			= orig.replace(/^query\s*/, "")
			,idxBracket = src.indexOf('{')
			,fnSig	= (idxBracket > -1) ? src.substr(0, idxBracket) : undefined
			,queryFn,parsed, name, args;
		if( fnSig ) {
			fnSig	= fnSig.match(queryTopRegExp);
			if( fnSig ) {
				fnSig = fnSig[0];
				parsed = fnSigParser.parse(fnSig);
				src = src.replace(fnSig, "").replace(/^\s*|\s*$/g, "");
				if (utils.findNextToken(src) === "{") {
					name		= parsed[0][0];
					args		= parsed[1];
