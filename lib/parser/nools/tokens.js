...
var utils = require("./util.js"),
	...
	parser = require('../constraint/parser.js');
...
var queryTopRegExp = /^([$A-Z_][0-9A-Z_$]*\s*\(.*\))\s*{$/i;		// captures up to the leading '{' 
...
var topLevelTokens = {
	...
	"query": function (orig, context, parse) {
		var src			= orig.replace(/^query\s*/, "")
			,leadSeg	= src.match(queryTopRegExp)
			,queryFn	= leadSeg ? leadSeg[1] : undefined
			,parsed, name, args;
		//
		if( leadSeg ) {
			parsed = parser.parse(queryFn);
			src = src.replace(queryFn, "").replace(/^\s*|\s*$/g, "");
			if (utils.findNextToken(src) === "{") {
				name = parsed[0][0];
				args = parsed[0][1];
				var rule = { name: name, options: {arguments:args}, constraints: null, action: null };
				var body = utils.getTokensBetween(src, "{", "}", true).join("");
				src = src.replace(body, "");
