    ...
    instance: {
        ...
        rule: function (rule) {
			...
			if(rule.pattern.queryName) {	
				this.queryResults[rule.pattern.queryName] = {};			// the hashCode is by parameter fact id(s)												
																		// supply the query entry point
				this.__queries[rule.pattern.queryName] = function() {	
					var ret = rule.pattern.queryFn.apply(me, arguments);			// add the sessions context
					return ret;
				}
			}
			this.rootNode.assertRule(rule,  me);
        },
		...
		getQuery: function(name) {
			return this.__queries[name];
		},

		getParamNode: function(pattern) {
			var paramNodes = this.rootNode.paramNodes,
				target;
			paramNodes.some(function(node) {
