var comb = require("comb"),
    ...
    atoms = require("./atoms"),
    EqualityAtom = atoms.EqualityAtom,
    HashAtom = atoms.HashAtom,
    ReferenceAtom = atoms.ReferenceAtom,
    CompositeAtom = atoms.CompositeAtom,
    rules = require("./rule"),
    Rule = rules.Rule,
    InitialRule = rules.InitialRule,
    atoms = require("./atoms");


var MatchResult = comb.define(null, {
    instance:{
        ...
        merge:function (mr) {
            ...
            comb.merge(ret.factHash, this.factHash, mr.factHash);
            ret.recency = this.recency.length ? this.recency : mr.recency;
            return ret;
        },
...
var NetworkBucket = comb.define(null, {
    instance:{
        counter:0
    }
});

var RootNode = comb.define(null, {
    instance:{
        ...
        __createJoinNode:function (pattern, outNode, side) {
            var joinNode;
            if (comb.isInstanceOf(pattern.rightPattern, NotPattern)) {
                joinNode = new NotNode()
            } else {
                joinNode = new JoinNode()
            }
            var parentNode = joinNode;
            if (comb.isInstanceOf(outNode, JoinNode)) {
        ...
        __buildNetwork:function (pattern, outNode, side) {
            if (comb.isInstanceOf(pattern, ObjectPattern)) {
                if (comb.isInstanceOf(pattern, NotPattern) && (!side || side === "left")) {
            ...
            } else if (comb.isInstanceOf(pattern, CompositePattern)) {
        ...
        __createAtomNode:function (pattern, outNode, side) {
            ...
            atoms.slice(1).forEach(function (atom) {
                var node;
                if (comb.isInstanceOf(atom, HashAtom)) {
                    node = this.__createPropertyNode(atom);
                } else if (comb.isInstanceOf(atom, ReferenceAtom)) {
    ...
    instance:{
        ...
        __propagate:function (method, assertable, outNodes) {
            ...
            outNodes.forEach(function (entry) {
                ...
                if (assertable.paths) {
                    var continuingPaths = comb.array.intersect(paths, assertable.paths);
...
var HashedNode = comb.define(AtomNode, {
    instance:{
        constructor:function (atom) {
            this._super([atom]);
...
var TypeNode = comb.define(HashedNode, {
    instance:{
        hashBy:function (atom) {
        ...
        factToHash:function (fact) {
            return fact.object._static || fact.object.constructor;
        },
        ...
        __propagate:function (method, assertion, outNodes) {
            (outNodes || this.nodes).forEach(function (e) {
                ...
                outNode[method](comb.merge(assertion, {factHash:{}, paths:paths}));
            })
        }
    }
});

var AlphaMemoryNode = comb.define(AtomNode, {
...
var PropertyNode = comb.define(AlphaMemoryNode, {
    instance:{
        assert:function (assertable) {
...
var ReferenceNode = comb.define(AtomNode, {
    instance:{
        match:function (leftContext, rightContext) {
            var match = leftContext.match.factHash;
            var fh = comb.merge({}, rightContext.match.factHash);
...
var EqualityNode = comb.define(AtomNode, {
    instance:{
        assert:function (assertable) {
            var fh = {};
...
var BaseBridgeNode = comb.define(Node, {
    instance:{
...
var BridgeNode = comb.define(BaseBridgeNode, {
    ...
    instance:{
        assert:function (assertable) {
            ...
            this.pattern.atoms.forEach(function (atom) {
                ...
                } else if (comb.isInstanceOf(atom, HashAtom)) {
                    var hash = atom.variables;
                    for (var i in hash) {
                        mr.factHash[hash[i]] = fact.object[i];
                    }
                } else {
                    atom.variables.forEach(function (v) {
                        if (atom.alias != v) {
                            mr.factHash[v] = fact.object[v];
                        }
                    })
                }
            });
...
var LeftAdapterNode = comb.define(Node, {
    instance:{
...
var RightAdapterNode = comb.define(Node, {
    instance:{
...
var NotNode = comb.define(JoinNode, {
    instance:{
...
var TerminalNode = comb.define(Node, {
    instance:{
        ...
        retract:function (fact) {
            this.activations.forEach(function (es) {
