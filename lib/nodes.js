...
define(null, {
    instance:{
        ...
        __createTypeNode:function (pattern) {
            var ret = new TypeNode(pattern.atoms[0]);
            var atoms = this.typeNodes, i = atoms.length - 1;
            for (; i >= 0; i--) {
                var n = atoms[i];
                if (ret.shareable(n)) {
                    return  n;
        ...
        __buildNetwork:function (pattern, outNode, side) {
            if (isInstanceOf(pattern, ObjectPattern)) {
                if (isInstanceOf(pattern, NotPattern) && (!side || side === "left")) {
                    return this.__buildNetwork(new CompositePattern(new InitialFactPattern(), pattern), outNode, side);
                }
...
var TypeNode = define(AlphaNode, {
    instance:{
        ...
        assert:function (fact) {
            if (this.atom.assert(fact.object)) {
        ...
        toString:function () {
            return "Type Node" + this.atom.constraint;
        },
...
var PropertyNode = define(AlphaNode, {
    instance:{
        ...
        assert:function (assertable) {
            var fh = {}, atom = this.atom, o = assertable.fact.object, alias = atom.alias;
            fh[alias] = o;
            if (atom.assert(fh)) {
...
var EqualityNode = define(AlphaNode, {
    instance:{
        ...
        assert:function (assertable) {
            var fh = {}, atom = this.atom;
            fh[atom.alias] = assertable.fact.object;
            if (atom.assert(fh)) {
                this._super([assertable])
            }
        },
