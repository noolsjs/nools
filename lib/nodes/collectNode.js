...
/**
	The collect CE identifies set variables from 'regular' bindings.  For any given hashCode if we remove set
	variables we have a SetHashCode for a given collection ( can be the empty string which means there are only set vars). 
	We store collections by this 'pruned' hash code. Subsequent lhs propagations map to particular collections since the
	non set bindings are unique per collection.  In this way we are partitioning the relation on the non-set variables.
		    
	Mod's cause the correct behavior in both the lhs and terminal nodes.  Retractions on set vars
	cause modifications unless the collection length goes to zero in which case an actual retraction is propagated.
	Obviously when collections are started an assert is propagated to start the collection.  
*/
FromNode.extend({
    instance: {
        ...
        __createMatches: function (lc) {
			...
			if(!rc) {
				//
                collection	= me.isCollectionObject ? {} : [];		// either an array [obj, obj,...] or { key: [], key2: [] }					
                collFact	= me.workingMemory.getFactHandle(collection, true);
			...
			me.setAliases.forEach( function(alias, i) {
				...
				if( me.isCollectionObject ) {							// { key1: [values], key2: [values], ... }
					...
					idx = _.findIndex( collection[alias], lcValue);	
					-1 === idx ? collection[alias].push(lcValue) : undefined;	
				}
				else {													// the collection's an array
					idx = _.findIndex(collection, lcValue);				// don't reproduce values already in the collection
		...
		*/
				...
				try { 
					me.setAliases.forEach(function(alias) {
						...
						if( me.isCollectionObject ) {					// a hash of sets -> { key: [values], key2: [values], ... }
							...
							idx		 = _.findIndex(theArray, lcOldValue);
							...
							if( 1 === me.setAliases.length ) {			// a simple array
								idx = _.findIndex(collection, lcOldValue);
								retract ? delete collection[idx] :  collection[idx] = lcNewValue;
							}
							else {										// we have a hash remove the entire thing from the array
								idx = _.findIndex(collection, x => x.filteredHashCode === fc.hashCode);
								_.remove(collection, collection[idx]);
								if(!retract) {
									if(!setHash) {
        ...
        ,getFilteredContext: function(lc) {
            ...
            lc.match.aliases.forEach(function(alias, i) {
				var idx = _.findIndex(me.setAliases, x => x == alias )
					,fact;
                if(	idx === -1 ) {	
			...
			_.forEach(match.factHash, (val, key) => clone.set(key, val));
			return clone;
        }
		//
		// clone the (left) context and remove the set oriented variables
        ...
        ,getFilteredHashCode: function(context) {
            ...
            context.match.aliases.forEach(function(alias, i) {
                if(	_.findIndex(me.setAliases, x => x == alias ) === -1  ) {	
                    facts.push(context.factHash[alias]);
                }
			});
			return facts.map(x => x.id).reduce(function(prev, cur, idx, array) { return prev ? ( prev + ':' + cur ) :  ('' + cur) });
        }
