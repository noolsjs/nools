...
declare({
    instance: {
        ...
        assertRule: function (rule, session) {
            var terminalNode;
			if(rule.pattern.queryName) {
				terminalNode = new QueryNode(this.bucket, this.__ruleCount++, rule, this.agendaTree);
			}
			else {
				terminalNode = new TerminalNode(this.bucket, this.__ruleCount++, rule, this.agendaTree);
			}
            this.__addToNetwork(rule, rule.pattern, terminalNode);
            this.__mergeJoinNodes();
			this.__linkFromNodes(session);
		...
		__linkFromNodes: function(session) {
			var me  = this;
			me.fromNodes.forEach(function(node) { node.session = session; } );
		},
        ...
        __createJoinNode: function (rule, pattern, outNode, side) {
			...
			} else if (pattern.rightPattern instanceof ParamPattern ) {
                joinNode = new ParamNode(pattern.rightPattern, this.workingMemory);
				this.paramNodes.push(joinNode);				// remember so query fn can inject parameter(s) initiating prop
				joinNode.pattern = pattern.rightPattern;	// must have pattern instance so node can be located
            } else if (pattern.rightPattern instanceof FromPattern) {
                joinNode = new FromNode(pattern.rightPattern, this.workingMemory);
				this.fromNodes.push(joinNode);
        ...
        __addToNetwork: function (rule, pattern, outNode, side) {
            ...
            if (pattern instanceof ObjectPattern) {
                if (!(pattern instanceof InitialFactPattern) && (!side || side === "left")) {
					initialPattern = new CompositePattern(new InitialFactPattern(), pattern);
                    this.__createBetaNode(rule, initialPattern, outNode, side);
                } else {
        ...
        __createAlphaNode: function (rule, pattern, outNode, side) {
            ...
            if (!(pattern instanceof FromPattern)) {
				if( rule.pattern.queryName && pattern instanceof InitialFactPattern ) {
					pattern.queryInitialFact = true;			// used to ensure initial fact propagates before queries are used in other rule LHS
				}
