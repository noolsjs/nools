    ...
    it.describe("#createRule", function (it) {
        ...
        it.describe("with strings", function (it) {
            it.should("create for string", function () {
                ...
                assert.equal(pattern.atoms[0].constraint, String);
            ...
            it.should("create for number", function () {
                ...
                assert.lengthOf(pattern.atoms, 2);
                assert.instanceOf(pattern.atoms[0], atom.ObjectAtom);
                assert.equal(pattern.atoms[0].constraint, Number);
                assert.instanceOf(pattern.atoms[1], atom.TrueAtom);
                assert.strictEqual(rule.cb, cb);
            });
            ...
            it.should("create for date", function () {
                ...
                assert.lengthOf(pattern.atoms, 2);
                assert.instanceOf(pattern.atoms[0], atom.ObjectAtom);
                assert.equal(pattern.atoms[0].constraint, Date);
                assert.instanceOf(pattern.atoms[1], atom.TrueAtom);
                assert.strictEqual(rule.cb, cb);
            });
            ...
            it.should("create for array", function () {
                ...
                assert.lengthOf(pattern.atoms, 2);
                assert.instanceOf(pattern.atoms[0], atom.ObjectAtom);
                assert.equal(pattern.atoms[0].constraint, Array);
                assert.instanceOf(pattern.atoms[1], atom.TrueAtom);
                assert.strictEqual(rule.cb, cb);
            });
            ...
            it.should("create for boolean", function () {
                ...
                assert.lengthOf(pattern.atoms, 2);
                assert.instanceOf(pattern.atoms[0], atom.ObjectAtom);
                assert.equal(pattern.atoms[0].constraint, Boolean);
                assert.instanceOf(pattern.atoms[1], atom.TrueAtom);
                assert.strictEqual(rule.cb, cb);
            });
            ...
            it.should("create for regexp", function () {
                ...
                assert.lengthOf(pattern.atoms, 2);
                assert.instanceOf(pattern.atoms[0], atom.ObjectAtom);
                assert.equal(pattern.atoms[0].constraint, RegExp);
                assert.instanceOf(pattern.atoms[1], atom.TrueAtom);
                assert.strictEqual(rule.cb, cb);
            });
            ...
            it.should("create for object", function () {
                ...
                assert.lengthOf(pattern.atoms, 2);
                assert.instanceOf(pattern.atoms[0], atom.ObjectAtom);
                assert.equal(pattern.atoms[0].constraint, Object);
                assert.instanceOf(pattern.atoms[1], atom.TrueAtom);
                assert.strictEqual(rule.cb, cb);
            });
        ...
        it.describe("with functions", function (it) {
            ...
            it.should("create for custom functions", function () {
                ...
                assert.lengthOf(pattern.atoms, 2);
                assert.instanceOf(pattern.atoms[0], atom.ObjectAtom);
                assert.equal(pattern.atoms[0].constraint, MyObject);
                assert.instanceOf(pattern.atoms[1], atom.TrueAtom)
                assert.strictEqual(rule.cb, cb);
            });
        });
        ...
        it.should("should create a composite rule", function () {
            ...
            var atoms = pattern.leftPattern.atoms;
            assert.lengthOf(atoms, 2);
            assert.instanceOf(atoms[0], atom.ObjectAtom)
            assert.instanceOf(atoms[1], atom.TrueAtom)
            var atoms = pattern.rightPattern.atoms;
            assert.lengthOf(atoms, 2);
            assert.instanceOf(atoms[0], atom.ObjectAtom)
            assert.instanceOf(atoms[1], atom.ReferenceAtom)
            assert.strictEqual(rule.cb, cb);
        });
        ...
        it.should("should create a or pattern", function () {
            ...
            for (var i = 0; i < 2; i++) {
                ...
                assert.lengthOf(atoms, 2);
                assert.instanceOf(atoms[0], atom.ObjectAtom)
                assert.instanceOf(atoms[1], atom[i == 0 ? "ReferenceAtom" : "EqualityAtom"]);
                assert.strictEqual(rule.cb, cb);
            }
        });
    });
